<template>
	<view v-if="page == 0" class="content">
		...
		<view class="introduction">
			<view class="intro-item">
				<view class="character">O</view>
				<view class="chr-intro chr-title">目标：</view>
			</view>
			<view class="chr-intro-zh">目标是要有野心的，有一些挑战的，有些让你不舒服的。</view>
			<view class="intro-item" style="margin-top: 50rpx;">
				<view class="character">KR</view>
				<view class="chr-intro chr-title">关键成果：</view>
			</view>
			<view class="chr-intro-zh">基于数值的期望描述能够产生可量化、可评分的结果。</view>
		</view>
		<button style="margin-bottom: 50rpx;" @click="next">开始制作</button>
	</view>
	<view v-else-if="page == 1" class="content">
		<view style="width: 100%;margin-top: 20rpx;">
			<view class="step-item">
				<view class="step-title" style="margin: 0;">
		...
		<view style="width: 100%;">
			<view class="step-item">
				...
				<view style="width: 100%;">
					...
					<image v-else-if="hasEmpty" src="../../static/添加.png" style="height: 40rpx;width: 40rpx;" mode="aspectFill" @click="add"></image>
				</view>
			</view>
		</view>
		<view style="width: 100%;">
			<view class="step-item">
				...
				<!-- <view style="width: 100%;">
					<picker-view :value="date" @change="bindChange">
						<picker-view-column>
		...
		<view class='btn'>
			<button style="margin-bottom: 50rpx;" @click="back">上一步</button>
			<button style="margin-bottom: 50rpx;" @click="next">下一步</button>
		</view>
	</view>
	<view v-else-if="page == 2" class="content">
		<view style="width: 100%;margin-top: 20rpx;">
			<view class="step-item">
				<view class="step-title" style="margin: 0;">
			...
			<view class="step-item">
				...
				<view style="width: 100%;flex-wrap: wrap;margin-top: 40rpx;" class="flex-row center-start">
					<view class="flex-column center-center child" v-for="(item,index) in background" :key="index" @click="chooseIt(index)">
						<view :style="{backgroundColor:item.color}" style="border-radius: 30rpx;height: 18vw;width: 18vw;position: relative;">
		...
		<view class='btn'>
			<button style="margin-bottom: 50rpx;" @click="back">上一步</button>
			<button style="margin-bottom: 50rpx;" @click="makePic">一键生成</button>
		</view>
	</view>
	<view v-else class="content" style="width: 100%;overflow: hidden;height: 100%;">
		<view class="cardBody">
			<view class="flex-column center-center" style="width: 100%;height: 100%;">
				...
				<image v-else style="position: absolute;z-index: 1000;top: 0;left: 50%;transform: translateX(-50%);" :src="canvasBase"
				 :style="{height: canvasHeight+'px',width: canvasWidth+'px'}" mode="aspectFit"></image>
			</view>
		</view>
		<view v-if="!load" class="flex-row center-center" style="height: 10%;">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			console.log(res)
			this.canvasHeight = res.windowHeight * 0.8 / this.canvasMaxHeight * this.canvasMaxWidth > res.windowWidth ? res.windowWidth /
				this.canvasMaxWidth * this.canvasMaxHeight : res.windowHeight * 0.8;
		...
		methods: {
			...
			draw(res, ctx) {
				...
				//title
				this.writeText(ctx, 'center', this.background[this.chooseId].deepColor, 60, '我的OKR', 300, 124);

				//执行人
				this.writeText(ctx, 'left', this.background[this.chooseId].deepColor, 35, '对象', 42, 225);

				this.writeText(ctx, 'left', '#404146', 25, this.name, 42, 281);

				//目标
				this.writeText(ctx, 'left', this.background[this.chooseId].deepColor, 35, '目标', 42, 369);

				this.writeText(ctx, 'left', '#404146', 25, this.objective, 42, 425);
				...
				for (let i = 0; i < this.key_res.length; i++) {
					this.writeText(ctx, 'left', '#404146', 25, (i + 1) + '. ' + this.key_res[i].value, 42, top + 53 * i);
				}
				...
				this.writeText(ctx, 'left', '#404146', 25, this.date, 42, top + 56);
